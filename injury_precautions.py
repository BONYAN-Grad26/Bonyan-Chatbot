...
INJURY_ENTITIES = {
    "shoulder": ["shoulder", "كتف", "الترقوة", "الهنج"],
    "back": ["back", "ضهر", "قطنية", "عمود فقري"],
    "knee": ["knee", "ركبة", "صابونة"],
    "elbow": ["elbow", "كوع", "المرفق"],
    "wrist": ["wrist", "ريست", "معصم", "إيد"],
    "neck": ["neck", "رقبة", "العنق"],
    "chest": ["chest", "صدر", "البنش", "القفص الصدري"],
    "foot": ["foot", "انكل", "كاحل", "رجل", "مفصل القدم"],
}
...
def handle(user_input, entity, is_short_func, user_data={}):
    ...
    if not data:
        return f"[injury_precautions | entity: {entity}] - البيانات مش موجودة"

    risk = "، ".join(data["risk_exercises"])
    mistakes = "\n- ".join(data["common_mistakes"])
    prevention = "\n- ".join(data["prevention"])
    tips = "\n- ".join(data["protection_tips"])
    warnings = "\n- ".join(data["warning_signs"])

    return (
        f"تمام بص دي التمارين اللي محتاج تاخد بالك منها وانت بتلعبها عشان تحمي {entity} ده:\n{risk}\n\n"
        f"ودي بعض الأخطاء الشائعة:\n- {mistakes}\n\n"
        f"ودي طريقة التصحيح والتفادي:\n- {prevention}\n\n"
        f"ودي نصايح للحماية:\n- {tips}\n\n"
        f"وأول حاجة تعملها لو حصل إيه:\n{data['first_aid']}\n\n"
        f"والعلامات اللي تروح فيها للدكتور:\n- {warnings}"
    )